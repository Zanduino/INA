...
inaDet::inaDet(inaEEPROM &inaEE)
{
  ...
  case INA237:
    power_LSB = (uint32_t)25 * current_LSB; // issue #66 corrected multiplier
    busVoltageRegister = INA237_BUS_VOLTAGE_REGISTER;
...
void INA_Class::readInafromEEPROM(const uint8_t deviceNumber)
{
  ...
  if (deviceNumber == _currentINA || deviceNumber > _DeviceCount)
    return; // Skip if correct device
            ...
            else
            {
              ...
              else
              {
                ...
                else
                {
                  if (tempRegister == 0)
                  {
                    tempRegister = readWord(INA237_MANUFACTURER_ID_REGISTER, deviceAddress); // Read the manufacurer register on INA237
                    if (tempRegister == INA_MANUFACTURER_ID)
                    {
                      inaEE.type = INA237;
                    }
                    else
...
bool INA_Class::conversionFinished(const uint8_t deviceNumber)
{
  ...
  */
  if (_DeviceCount == 0)
    return false;                                 // Return finished if invalid device. Issue #65
  ...
  for (uint8_t i = 0; i < _DeviceCount; i++) // Loop for each device found
  {
    if (deviceNumber == UINT8_MAX || deviceNumber == i) // If this device needs setting
    {
      ...
      case INA260:
        ...
        if (alertState)
          bitSet(alertRegister, INA_ALERT_CONVERSION_RDY_BIT);           // Turn on the bit
